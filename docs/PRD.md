...
| 设置 | 刮削器管理 | 管理各个刮削器的配置（超时时间、重试次数、API参数） |
| 设置 | 刮削器总管理 | 配置歌曲标签、封面、歌词使用哪些刮削器，信息冲突处理策略 |
...
| 设置 | 刮削器管理 | 刮削器列表、启用/禁用、优先级调整、配置超时时间/重试次数/API参数 |
| 设置 | 刮削器总管理 | 配置歌曲标签、封面、歌词使用哪些刮削器 |
...
| FR-011-D | 支持 QQ 音乐刮削器 API | 高 |
| FR-011-E | 支持 Spotify 刮削器 API | 中 |
| FR-011-F | 支持 iTunes 刮削器 API | 中 |
| FR-011-G | 支持咪咕音乐刮削器 API | 高 |
| FR-011-H | 支持酷狗音乐刮削器 API | 高 |
| FR-011-I | 支持配置启用/禁用各个刮削器 | 高 |
| FR-011-J | 支持配置刮削器优先级顺序 | 中 |
| FR-011-K | 支持配置每个刮削器的请求间隔时间 | 低 |
| FR-011-L | 支持配置每个刮削器的超时时间 | 中 |
| FR-011-M | 支持配置每个刮削器的重试次数 | 中 |
| FR-011-N | 支持配置每个刮削器的自定义API参数 | 中 |
| FR-011-O | 支持为不同数据类型（标签/封面/歌词）配置不同的刮削器组合 | 高 |
| FR-011-P | 支持使用 Acoustid 进行音乐指纹匹配获取标题、艺术家、专辑信息 | 高 |
| FR-011-Q | 支持配置信息冲突时优先使用原文件标签或刮削数据 | 高 |
